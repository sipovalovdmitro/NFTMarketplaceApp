module UsersHelper
  def link_to_image(user)
    return if user.blank? && user.address.blank?
    if user.is_verified?
      link_class = "author__avatar author__avatar--verified"
    else
      link_class = "author__avatar"
    end
    link_to user_path(user.address), class: link_class do
