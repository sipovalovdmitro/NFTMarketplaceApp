class User < ApplicationRecord
  ...
  before_create :set_approved, :set_un_verified
  ...
  def banner_image(width=nil, height=nil)
    if banner.attached?
      if width || height
        banner.variant(resize_to_limit: [width, height])
      else
        banner
      end
    else
  ...
  def get_collections(tab, filters, page_no, user)
    case tab
    when "collectibles"
    ...
    when "owner"
      collections.paginate(page: page_no)
    when "my_bids"
      Collection.joins(:bids).where(bids: {user_id: user.id})
    when "liked"
    ...
    when "nft_collections"
        Api::NftCollections::nft_collections(user.address, page_no)
    else
      Collection.where("owner_id=? and instant_sale_enabled=?", id, true).includes(attachment_attachment: :blob, creator: { attachment_attachment: :blob }, owner: { attachment_attachment: :blob }).paginate(page: page_no)
    end
  end
